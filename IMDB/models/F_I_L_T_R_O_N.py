...
class F_I_L_T_R_O_N:
    ...
    def filter_by_category(self, categories_as_string: str):
    ...
    def filter_by_director(self, directors_as_string: str):
    ...
    def filter_by_length(self, minLength: int, maxLength: int):
        for mov in self.movies:
            if minLength <= mov.Length <= maxLength:
                self.result.append(mov)

    def filter_by_year(self, minYear: int, maxYear: int):
        for mov in self.movies:
